...
//		}
	...
	public TreeNode constructBST_util_better(int[] preOrderArr, Index preIndex, int key, int min, int max, int size) {
		if(preIndex.index >= size || (min > max)) return null;
		...
		if(key > min && key < max) {
			...
			if (preIndex.index < size) {
				// Do left
				node.left = constructBST_util_better(preOrderArr, preIndex, preOrderArr[preIndex.index], min, key, size);

				// Do right
				node.right = constructBST_util_better(preOrderArr, preIndex, preOrderArr[preIndex.index], key, max, size);
			}
		}
